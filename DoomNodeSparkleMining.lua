-- DoomNodeSparkle - Mining
-- Shows a pulsing sparkle near the cursor when mousing over a mining node tooltip.

local Sparkle = CreateFrame("Frame", "DoomMiningSparkleFrame", UIParent)
Sparkle:SetWidth(48)
...
local function TooltipTextLooksLikeMiningNode()
  if DoomNodeSparkle_Settings and not DoomNodeSparkle_Settings.mining then
    return false
  end
  ...
  for i = 2, GameTooltip:NumLines() do
    ...
    if line then
      ...
      if txt then
        ...
        if string.find(txt, "Mining") or string.find(txt, "Requires Mining") then
          return true
        end
      end
    end
...
if GameTooltip and GameTooltip.HookScript then
  GameTooltip:HookScript("OnShow", function()
    if TooltipTextLooksLikeMiningNode() and not sparkleShown then
      ShowSparkleAtCursor()
      sparkleShown = true
    end
  end)
