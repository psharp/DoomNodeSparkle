-- DoomNodeSparkle - Herbalism
-- Shows a pulsing sparkle near the cursor when mousing over an herb node tooltip.

local Sparkle = CreateFrame("Frame", "DoomHerbSparkleFrame", UIParent)
Sparkle:SetWidth(48)
  ...
  for i = 2, GameTooltip:NumLines() do
    ...
    if line then
      ...
      if txt then
        ...
        if string.find(txt, "Herbalism") or string.find(txt, "Requires Herbalism") then
          return true
        end
      end
    end
...
if GameTooltip and GameTooltip.HookScript then
  GameTooltip:HookScript("OnShow", function()
    if TooltipTextLooksLikeHerbNode() and not sparkleShown then
      ShowSparkleAtCursor()
      sparkleShown = true
    end
  end)
  ...
  Poll:SetScript("OnUpdate", function()
    if GameTooltip and GameTooltip:IsShown() and TooltipTextLooksLikeHerbNode() then
